...
export type LogType = 'log' | 'eloquent_model' | 'executed_query' | 'mailable' | 'application_log' | 'event' | 'table' | 'info' | 'debug' | 'warning' | 'error'
...
export const logTypes: Record<LogType, { label: string; color: string; border: string; dot: string }> = {
  ...
  application_log: { label: 'App', color: 'text-green-400', border: 'border-green-500/30', dot: 'bg-green-500' },
  event: { label: 'Event', color: 'text-teal-400', border: 'border-teal-500/30', dot: 'bg-teal-500' },
  table: { label: 'Table', color: 'text-indigo-400', border: 'border-indigo-500/30', dot: 'bg-indigo-500' },
